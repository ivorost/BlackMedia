...
public extension Network.Setup {
    class Test : Capture.Setup.Vector {
        ...
        public init(views: Network.TestView) {
            self.views = views
            super.init()
        }
        ...
        public override func create() -> [Capture.Setup.Proto] {
            ...
            let websocket = Network.Setup.WebSocket(data: self, url: wsSenderData, target: .capture)
...
public class NetworkTestView : NSObject {
    ...
    @IBAction func startAction(_ sender: Any) {
        ...
        let newSession = Network.Setup.Test(views: self).setup()
