...
public extension Video.Setup {
    class Receiver : Vector {
        ...
        public init(root: Proto, url: URL, view: View, layer: SampleBufferDisplayLayer) {
            self.root = root
            self.url = url
            self.view = view
            self.layer = layer
        ...
        public override func create() -> [Proto] {
            ...
            let webSocketHelm = Checkbox(
                next: cast(video: Network.Setup.WebSocket(helm: root, url: url, target: .serializer)),
            ...
            let timestamp1video = Processor(kind: .preview, video: timestamp1processor.video)
